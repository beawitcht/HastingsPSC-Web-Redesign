...
document.addEventListener("DOMContentLoaded", function () {
    ...
    loadMoreBtn.addEventListener('click', () => {
        const nextBatch = Array.from(cards).slice(shown, shown + BATCH_SIZE);
    ...
    loadMoreBtnArticle.addEventListener('click', () => {
        const nextBatch = Array.from(articleCards).slice(shown, shown + BATCH_SIZE);
        nextBatch.forEach(articleCards => articleCards.classList.remove('hidden'));
        shown += nextBatch.length;

        if (shown >= articleCards.length) {
